...
**Items** (`items/`):
- `manuelle-trigger-erkennung-deadman.items`
- `manuelle-trigger-erkennung-proxy.items`
- `manuelle-trigger-erkennung-timestamp.items`

**Regeln** – bitte nur EINE der drei Varianten verwenden:

- Rules DSL (`rules-dsl/`): `manuelle-trigger-erkennung-deadman.rules`, `manuelle-trigger-erkennung-proxy.rules`, `manuelle-trigger-erkennung-timestamp.rules`
- JavaScript Scripting (`rules-js/`): `manuelle-trigger-erkennung-deadman.js`, `manuelle-trigger-erkennung-proxy.js`, `manuelle-trigger-erkennung-timestamp.js`
- Python 3 Scripting (`rules-python/`): `manuelle_trigger_erkennung_deadman.py`, `manuelle_trigger_erkennung_proxy.py`, `manuelle_trigger_erkennung_timestamp.py`
