...
**Items** (`items/`):
- `decoupled-notification-service.items`

**Regeln** – bitte nur EINE der drei Varianten verwenden:

- Rules DSL (`rules-dsl/`): `decoupled-notification-service-sender.rules`, `decoupled-notification-service.rules`
- JavaScript Scripting (`rules-js/`): `decoupled-notification-service-sender.js`, `decoupled-notification-service.js`
- Python 3 Scripting (`rules-python/`): `decoupled_notification_service.py`, `decoupled_notification_service_sender.py`
