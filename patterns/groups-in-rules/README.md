...
**Items** (`items/`):
- `groups-in-rules.items`

**Regeln** – bitte nur EINE der drei Varianten verwenden:

- Rules DSL (`rules-dsl/`): `groups-in-rules.rules`
- JavaScript Scripting (`rules-js/`): `groups-in-rules.js`
- Python 3 Scripting (`rules-python/`): `groups_in_rules.py`
