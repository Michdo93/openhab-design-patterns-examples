...
Nicht-blockierende Schleifen-Timer (Deckenventilator-Beispiel) statt while-Schleifen, inkl. Expire-Binding-Alternative.

## Dateien in diesem Beispiel

**Items** (`items/`):
- `looping-timers-expire.items`

**Regeln** – bitte nur EINE der drei Varianten verwenden:

- Rules DSL (`rules-dsl/`): `looping-timers-expire.rules`, `looping-timers.rules`
- JavaScript Scripting (`rules-js/`): `looping-timers-expire.js`, `looping-timers.js`
- Python 3 Scripting (`rules-python/`): `looping_timers.py`, `looping_timers_expire.py`
