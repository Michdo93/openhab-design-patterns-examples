...
**Items** (`items/`):
- `proxy-item-garage.items`
- `proxy-item.items`

**Regeln** – bitte nur EINE der drei Varianten verwenden:

- Rules DSL (`rules-dsl/`): `proxy-item-garage.rules`, `proxy-item.rules`
- JavaScript Scripting (`rules-js/`): `proxy-item-garage.js`, `proxy-item.js`
- Python 3 Scripting (`rules-python/`): `proxy_item.py`, `proxy_item_garage.py`
