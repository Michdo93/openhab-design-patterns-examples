...
Schnelle Mehrfach-Events per Timer-Sperre entprellen, um Doppelaktionen zu vermeiden.

## Dateien in diesem Beispiel

**Items** (`items/`):
- `event-debouncer.items`

**Regeln** – bitte nur EINE der drei Varianten verwenden:

- Rules DSL (`rules-dsl/`): `event-debouncer.rules`
- JavaScript Scripting (`rules-js/`): `event-debouncer.js`
- Python 3 Scripting (`rules-python/`): `event_debouncer.py`
