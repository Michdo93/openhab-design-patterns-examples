...
Regeln zur Laufzeit aktivieren/deaktivieren (Weihnachtsmodus-Beispiel); DSL nur mit if-return-Workaround.

## Dateien in diesem Beispiel

**Regeln** – bitte nur EINE der drei Varianten verwenden:

- Rules DSL (`rules-dsl/`): `rule-deaktivierung.rules`
- JavaScript Scripting (`rules-js/`): `rule-deaktivierung.js`
- Python 3 Scripting (`rules-python/`): `rule_deaktivierung.py`
